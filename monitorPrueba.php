...
<section id="section2" class="colorfondo" >
  <div class="container" style="min-height: 600px;">
    <div class="row">
      <div class="col-xs-12">
        <?php 
		// trae los datos de la prueba con el nombre del instructor creador
		$pru = consultaSql("select * from prueba3 inner join usuario on prueba3.prucreador = usuario.usuId where pruid = $pruid limit 1");
        ...
        <a href="monitorPruebaExtendido.php?id=<?php echo $pruid ?>" class="divpo"> </a>
        <span  id="cambiaMonitor" class="divpo" style="font-size:0.9em;" data-toggle="tooltip" title="Click para cambiar al modo Extendido <br>(Consume mas recursos del Equipo)"> Monitor General de Prueba</span>
        
        </div>
        <div class="text-center" style="padding:0em 5em;"> <span style="font-size:1.7em"><?php echo $pru->nombre ?>&nbsp;</span>
        ...
        <div class="table-responsive">
          <table id="example" class="table table-striped table-bordered table-hover table-condensed ">
            <thead>
              <tr>
                <th style="vertical-align:middle;" class="text-center" rowspan="1" colspan="3"> <nobr> Filtrar por ficha
                  <select id="selFicha">
		 ...
		 while($res=$fichas->fetch_object()){?>
                  ...
                  &nbsp; <span id="cantAprendices" style="color:#494949"></span> </nobr> </th>
                <th style="vertical-align:middle;" class="text-center" rowspan="1" colspan="4"> Correctas /Erradas <br>
                  /Sin contestar   % Efectividad</th>
              </tr>
              <tr>
                ...
                <th style="vertical-align:middle;" class="text-center" >USUARIO <span class="glyphicon glyphicon-chevron-down divpo" id="ascUsuario"></span></th>
                <th style="vertical-align:middle;" class="text-center" ><nobr>Ficha <span class="glyphicon glyphicon-chevron-down divpo" id="ascFicha"></span></nobr></th>

                <th style="padding:0px" class="text-center"><nobr>CO <span class="glyphicon glyphicon-chevron-down divpo" id="ascCorrectas"></span></nobr></th>
                <th style="padding:0px" class="text-center"><nobr>ER <span class="glyphicon glyphicon-chevron-down divpo" id="ascErradas"></span></nobr></th>
                  </th>
                <th style="padding:0px" class="text-center"><nobr>SC <span class="glyphicon glyphicon-chevron-down divpo" id="ascSinContestar"></span></nobr></th>
                  </th>
		  ...
		  /*
          $resultados = consultaSql("select pruId,usuId,cantidadPreguntas,correctas,erradas,sinContestar,pruFechaInicio,pruFechaFin,
pruTiempo,horaInicio,horaFin,uInicio,uFin,uAhora,usuNumeroDoc,Nombres,Apellidos,ficha from consultor1 where pruId = $pruid $filtro $order;");
		  */
		 ...
		 while($res=$resultados->fetch_object()){?>
			...
			if($res->uFin < $res->uAhora and $res->uFin != 0) { 
                ...
                <td><span class="glyphicon glyphicon-list-alt" style="font-size:1.3em;<?php echo $color ?>" title="<?php echo $titulo ?>" data-toggle="tooltip" data-usuid = "<?php echo $res->usuId ?>"></span></td>
                <td><nobr> <span class="divpo" data-usuid = "<?php echo $res->usuNumeroDoc ?>"> <?php echo $res->Nombres." ".$res->Apellidos ?></span></nobr></td>
                <td><span style="color:rgba(126,126,126,1.00);"><?php echo $res->ficha ?></span></td>
                <td class="text-center" style="font-size:1.3em; color:#00AB1E"><b> <?php echo $res->correctas ?></b></td>
                <td class="text-center" style="font-size:1.3em; color:#FF2124"><?php echo $res->erradas ?></td>
                <td class="text-center" style="font-size:1.3em; color:#6B6B6B"><?php echo $res->sinContestar ?></td>
                <td class="text-center" style="font-size:1.3em; color:#4B4B4B"><?php echo resPorcen($res->correctas, $res->cantidadPreguntas) ?></td                
              >
              </tr>
              <?php }?>
            </tbody>
...
$("#cambiaMonitor").click(function(){
	window.open("monitorPruebaExtendido.php" + location.search, "_self");
	});
	...
	$('span[data-preid]').click(function (){
		...
		if (opcid != 0) {
			...
			$.ajax({
				url: "controlador/procesaModal.php", 
				type: "POST", 
				data: {proceso:"pregunta",id:id,opcid:opcid,pruid:pruid}, 
				success: function(datos){
					$('#mContenido').html(datos)
					$('.modal-title').text("Informacion de la Pregunta")
					$("#loadingAjax").fadeOut(300);	
